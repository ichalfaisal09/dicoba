...
#[Layout('layouts.app')]
class ImportTkp extends Component
{
    use WithFileUploads;
    ...
    public function render()
    {
        ...
        return view('livewire.admin.manajemensoal.tkp.import', [
            'variasiList' => $groupedVariasi,
        ])->layoutData([
            'title' => __('Import Soal TKP'),
        ]);
    }
    ...
    public function import(): void
    {
        ...
        if (empty($decoded)) {
            throw ValidationException::withMessages([
                'jsonFile' => [__('Berkas JSON tidak berisi soal TKP.')],
            ]);
        }
        ...
        session()->flash('callout', [
            ...
            'text' => __('Sebanyak :jumlah soal TKP berhasil diimpor.', ['jumlah' => count($preparedItems)]),
        ]);

        $this->resetForm();

        $this->redirectRoute('admin.manajemen-soal.tkp');
    }
    ...
    {
        $latest = SoalPertanyaan::where('kode_soal', 'like', 'TKP-%')
            ->latest('kode_soal')
            ->value('kode_soal');
