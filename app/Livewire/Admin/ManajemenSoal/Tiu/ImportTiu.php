...
#[Layout('layouts.app')]
class ImportTiu extends Component
{
    use WithFileUploads;
    ...
    public function render()
    {
        $variasi = KategoriVariasi::query()
            ->whereHas('materi.subtes', fn ($query) => $query->where('kode', 'TIU'))
            ->with('materi:id,nama')
            ->orderBy(
        ...
        return view('livewire.admin.manajemensoal.tiu.import', [
            'variasiList' => $groupedVariasi,
        ])->layoutData([
            'title' => __('Import Soal TIU'),
        ]);
    }
    ...
    public function import(): void
    {
        ...
        if (empty($decoded)) {
            throw ValidationException::withMessages([
                'jsonFile' => [__('Berkas JSON tidak berisi soal TIU.')],
            ]);
        }
        ...
        session()->flash('callout', [
            ...
            'text' => __('Sebanyak :jumlah soal TIU berhasil diimpor.', ['jumlah' => count($preparedItems)]),
        ]);

        $this->resetForm();

        $this->redirectRoute('admin.manajemen-soal.tiu');
    }
    ...
    {
        $latest = SoalPertanyaan::where('kode_soal', 'like', 'TIU-%')
            ->latest('kode_soal')
            ->value('kode_soal');
