<div class="flex h-full w-full flex-1 flex-col gap-8">
    ...
    <div
        class="relative overflow-hidden rounded-2xl border border-zinc-200 bg-gradient-to-br from-indigo-50 via-white to-blue-50 p-8 dark:border-zinc-700 dark:from-indigo-950/60 dark:via-zinc-900 dark:to-blue-950/40">
        <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div class="space-y-3">
                <flux:badge variant="outline" color="primary">
                    {{ __('Tryout Terbaru Siap Diikuti') }}
                </flux:badge>
                <flux:heading size="2xl">{{ __('Tingkatkan peluang kelulusanmu dengan latihan terstruktur') }}
                </flux:heading>
                ...
                <flux:modal name="konfirmasi-daftar-tryout" class="max-w-xl" wire:model="konfirmasiModal" focusable>
                    <div class="space-y-6">
                        ...
                        @if ($paketKonfirmasi)
                            <div class="rounded-2xl border border-zinc-200 p-4 dark:border-zinc-700">
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    <flux:badge>
                                        {{ __('Durasi :menit menit', ['menit' => $paketKonfirmasi['waktu_pengerjaan']]) }}
                                    </flux:badge>
                                </div>
                                ...
                                <div class="mt-4 grid gap-3 text-sm text-zinc-600 dark:text-zinc-300 sm:grid-cols-2">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span>{{ __('Biaya') }}:
                                            <strong
                                                class="text-zinc-800 dark:text-white">{{ number_format($paketKonfirmasi['harga'], 0, ',', '.') }}</strong>
                                        </span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span>{{ __('Status Sebelumnya') }}:
                                            <strong
                                                class="text-zinc-800 dark:text-white">{{ __($paketKonfirmasi['status'] ?? 'Belum terdaftar') }}</strong>
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex justify-end gap-3">
                            ...
                            <flux:button icon="sparkles" variant="primary" wire:click="registerConfirmed"
            ...
            <div class="relative w-full max-w-sm self-end md:self-center">
                <div
                    class="rounded-2xl border border-indigo-200 bg-white/70 p-6 shadow-xl ring-1 ring-indigo-200/60 dark:border-indigo-500/30 dark:bg-indigo-950/40 dark:ring-indigo-500/20">
                    ...
                    <div class="mt-4 flex flex-col gap-4 text-sm">
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                class="font-semibold text-indigo-600 dark:text-indigo-300">{{ $statistik['total_paket'] }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-zinc-500 dark:text-zinc-300">{{ __('Jadwal terdekat') }}</span>
                            <span
                                class="font-semibold">{{ $statistik['jadwal_terdekat'] ?? __('Belum ada jadwal') }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-zinc-500 dark:text-zinc-300">{{ __('Tryout diselesaikan') }}</span>
                            <span class="font-semibold">{{ $statistik['total_selesai'] }}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="space-y-4">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex flex-wrap gap-3">
                <flux:input icon="magnifying-glass" placeholder="{{ __('Cari berdasarkan nama paket') }}"
        ...
        <div class="grid gap-4 lg:grid-cols-3">
            @forelse ($paket as $item)
                <div
                    class="flex h-full flex-col justify-between rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm transition hover:border-indigo-200 hover:shadow-md dark:border-zinc-700 dark:bg-zinc-900">
                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                        ...
                        <div class="grid gap-2 text-sm text-zinc-500 dark:text-zinc-300">
                            <div class="flex items-center gap-2">
                                <flux:icon name="clock" class="size-4" />
                                <span>{{ __('Durasi') }}: <strong
                                        class="text-zinc-700 dark:text-white">{{ $item['waktu_pengerjaan'] }}
                                        {{ __('menit') }}</strong></span>
                            </div>
                            <div class="flex items-center gap-2">
                                <flux:icon name="currency-dollar" class="size-4" />
                                <span>{{ __('Biaya') }}: <strong
                                        class="text-zinc-700 dark:text-white">{{ number_format($item['harga'], 0, ',', '.') }}</strong></span>
                            </div>
                            <div class="flex items-center gap-2">
                                <flux:icon name="users" class="size-4" />
                                <span>{{ __('Peserta terdaftar') }}: <strong
                                        class="text-zinc-700 dark:text-white">{{ $item['bookings_count'] }}</strong></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 space-y-3 text-sm">
                        <div class="flex items-center gap-2 text-zinc-500 dark:text-zinc-300">
                            ...
                            <span>{{ __('Mulai') }}:
                                <strong
                                    class="text-zinc-700 dark:text-white">{{ $item['booking_mulai'] ?? __('Belum dijadwalkan') }}</strong></span>
                        </div>
                        <div class="flex items-center gap-2 text-zinc-500 dark:text-zinc-300">
                            <flux:icon name="calendar" class="size-5" />
                            <span>{{ __('Selesai') }}:
                                <strong
                                    class="text-zinc-700 dark:text-white">{{ $item['booking_selesai'] ?? __('Belum dijadwalkan') }}</strong></span>
                        </div>
                    </div>

                    <div class="mt-6 flex flex-col gap-3">
                        @if ($item['booking_status'] === \App\Models\TryoutBooking::STATUS_ACTIVE)
                            <flux:button icon="play" block :href="route('peserta.tryout.ujian', $item['booking_id'])" wire:navigate>
                                {{ __('Mulai Ujian') }}
                            </flux:button>
                        @else
                        ...
                        <flux:button variant="ghost" icon="eye" block
                            :href="route('peserta.tryout.detail', $item['id'])" wire:navigate>
            ...
            @empty
                <div
                    class="col-span-full rounded-2xl border border-dashed border-zinc-200 bg-white p-10 text-center dark:border-zinc-700 dark:bg-zinc-900">
