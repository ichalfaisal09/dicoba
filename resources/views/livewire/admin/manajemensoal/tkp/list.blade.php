<div class="flex h-full w-full flex-1 flex-col gap-6">
    ...
    <flux:card>
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="space-y-1">
                <flux:heading size="lg">{{ __('Daftar Soal TKP') }}</flux:heading>
                <flux:text variant="muted">
                    {{ __('Kelola soal Tes Karakteristik Pribadi. Daftar berikut menampilkan teks soal dan opsi aksi cepat.') }}
                </flux:text>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <flux:button icon="arrow-up-tray" variant="ghost"
                    :href="route('admin.manajemen-soal.tkp.import')" wire:navigate>
                    {{ __('Import JSON') }}
                </flux:button>
                <flux:button icon="plus" :href="route('admin.manajemen-soal.tkp.create')" wire:navigate>
                    {{ __('Tambah Soal') }}
                </flux:button>
            </div>
        </div>
        ...
        <div class="mt-6 space-y-">
            @if ($soal->isEmpty())
                <div
                    ...
                    <flux:heading size="md" class="mt-4">{{ __('Belum ada soal TKP') }}</flux:heading>
                    <flux:text variant="muted" class="mt-2">
                        {{ __('Tambahkan soal TKP terlebih dahulu untuk mulai mengelolanya.') }}
                    </flux:text>
                </div>
            @else
                <flux:table :paginate="$soal">
                    ...
                    <flux:table.rows>
                        @foreach ($soal as $item)
                            <flux:table.row wire:key="soal-tkp-{{ $item->id }}">
                                <flux:table.cell>
                                    <flux:text>{{ $item->teks_soal }}</flux:text>
                                </flux:table.cell>
                                <flux:table.cell class="flex justify-end gap-2">
                                    <flux:button size="sm" variant="ghost" icon="eye"
                                        :href="route('admin.manajemen-soal.tkp') . '/show/' . $item->id" wire:navigate>
                                        {{ __('Detail') }}
                                    </flux:button>
