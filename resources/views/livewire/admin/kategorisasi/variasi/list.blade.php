<div class="flex h-full w-full flex-1 flex-col gap-6">
    <flux:card>
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="space-y-1">
                <flux:heading size="lg">{{ __('Daftar Variasi') }}</flux:heading>
                <flux:text variant="muted">
                    {{ __('Kelola variasi soal untuk setiap materi agar pelatihan peserta lebih kaya.') }}
                </flux:text>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <flux:button icon="plus" :href="route('admin.kategorisasi.variasi.create')" wire:navigate>
                    {{ __('Tambah Variasi') }}
                </flux:button>
            </div>
        </div>
        ...
        <div class="mt-6">
            @if ($variasi->isEmpty())
                <div class="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-200 bg-zinc-50/60 px-8 py-16 text-center dark:border-white/10 dark:bg-white/5">
                    ...
                    <flux:heading size="md" class="mt-4">{{ __('Belum ada variasi') }}</flux:heading>
                    <flux:text variant="muted" class="mt-2">
                        {{ __('Tambahkan variasi baru untuk memperkaya bank soal pada materi terpilih.') }}
                    </flux:text>
                    <flux:button icon="plus" class="mt-6" :href="route('admin.kategorisasi.variasi.create')" wire:navigate>
                        {{ __('Tambah Variasi Pertama') }}
                    </flux:button>
                </div>
            @else
                <flux:table :paginate="$variasi">
                    <flux:table.columns>
                        <flux:table.column variant="strong">{{ __('Variasi') }}</flux:table.column>
                        <flux:table.column>{{ __('Materi') }}</flux:table.column>
                    ...
                    <flux:table.rows>
                        @foreach ($variasi as $item)
                            <flux:table.row wire:key="variasi-{{ $item->id }}">
                                ...
                                <flux:table.cell>
                                    <flux:badge variant="soft" color="neutral">
                                        {{ $item->materi?->nama ?? __('Tidak diketahui') }}
                                    </flux:badge>
                                </flux:table.cell>

                                <flux:table.cell>
                                    <flux:badge variant="outline" color="neutral">
                                        {{ $item->materi?->subtes?->nama ?? __('Tidak diketahui') }}
                                    </flux:badge>
                                </flux:table.cell>
